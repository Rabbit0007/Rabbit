...
from cairn.dispatcher.tasks import common, execute
...
def _local_payload() -> dict:
    return {
        ...
        "workers": [
            {"name": "local-claude", "type": "claudecode", "task_types": ["execute"], "max_running": 1, "priority": 0},
            {"name": "local-codex", "type": "codex", "task_types": ["execute"], "max_running": 1, "priority": 1},
            {"name": "local-pi", "type": "pi", "task_types": ["decide", "execute"], "max_running": 1, "priority": 2},
        ],
    }
...
def test_local_cli_check_passes_when_cli_present() -> None:
    payload = _local_payload()
    payload["workers"] = [{"name": "m", "type": "mock", "task_types": ["decide", "execute"], "max_running": 1, "priority": 0}]
    config = DispatchConfig.model_validate(payload)
...
def _bare_worker(worker_type: str) -> WorkerConfig:
    return WorkerConfig.model_validate(
        {"name": worker_type, "type": worker_type, "task_types": ["execute"], "max_running": 1, "priority": 0}
    )
...
def test_pi_local_driver_honors_explicit_cli_and_writes_provider_config(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(PiDriver, "_agent_dir", staticmethod(lambda _worker: str(tmp_path / "agent")))
    worker = WorkerConfig.model_validate(
        {
            ...
            "env": {
                ...
                "PI_API_KEY": "top-secret",
                "PI_PROVIDER_API": "openai-completions",
            },
        }
    )

    driver = PiDriver(local=True)
    argv = driver.build_decide(worker, "PROMPT", None).argv

    assert driver.local_binary(worker) == "/real/pi"
    assert "/real/pi" in argv
    assert "top-secret" not in " ".join(argv)
    assert "top-secret" in (tmp_path / "agent" / "models.json").read_text()
    assert argv[argv.index("--tools") + 1] == "read"
    assert (tmp_path / "agent" / "models.json").stat().st_mode & 0o777 == 0o600
    assert (tmp_path / "agent").stat().st_mode & 0o777 == 0o700

    execute_argv = driver.build_execute(worker, "PROMPT", None).argv
    assert execute_argv[execute_argv.index("--tools") + 1] == "read,write,edit,bash,grep,find,ls"
    ...
    return DispatchConfig.model_validate(
        {
            ...
            "tasks": {
                "decide": {"timeout": 30, "max_steps": 3},
                "execute": {"timeout": 30, "conclude_timeout": 10},
            },
            "workers": [
                {"name": name, "type": worker_type, "task_types": ["decide", "execute"], "max_running": 1, "priority": 0}
            ],
        }
    )
...
def test_explore_runs_real_local_cli_end_to_end(tmp_path: Path, monkeypatch) -> None:
    ...
    assert client.concluded == [("proj_001", "s001", "test-worker", "local fake fact")]
