...
def test_pi_driver_models_json_and_execute_argv_include_context_window_and_tools() -> None:
    ...
    assert models["providers"]["cairn"]["apiKey"] == "secret"
...
def test_decide_drivers_remove_world_mutating_tools() -> None:
    ...
    pi_argv = PiDriver().build_decide(pi_worker, "prompt", None).argv
    tools_index = pi_argv.index("--tools")
    assert pi_argv[tools_index + 1] == "read"
    ...
    codex_argv = CodexDriver().build_decide(codex_worker, "prompt", None).argv
