"""Backward-compat router: delegates all intents endpoints to steps."""
...
def heartbeat(project_id: str, intent_id: str, body: HeartbeatRequest):
    with get_conn() as conn:
        check_project_active(conn, project_id)
        get_claimable_open_step_or_404(conn, project_id, intent_id, body.worker)
...
def release(project_id: str, intent_id: str, body: HeartbeatRequest):
    with get_conn() as conn:
        check_project_active(conn, project_id)
        row = get_releasable_open_step_or_404(conn, project_id, intent_id, body.worker)
